...
class Bot:
    ...
    def bfs(self, gmap: Map, items, curr, acc, maxdepth, currdepth=0):
        if currdepth == maxdepth:
            return acc + self.values[curr[0]][curr[1]] * 0, None
        best = float("-inf")
        best_action = None
        directions = [Bot.up, Bot.right, Bot.down, Bot.left]
        for direction in directions:
            pos = direction(curr, gmap.size[0], gmap.size[0])
            if pos is None or not gmap.is_free(*pos):
                continue
            r, c = pos
            # copy items
            if items[r][c]:
                _items = [x[:] for x in items]
                _items[r][c] = 0
                res = self.bfs(
                    gmap,
                    _items,
                    pos,
                    acc + items[r][c] * Bot.GAMMA ** currdepth,
                    maxdepth,
                    currdepth + 1,
                )
            else:
                res = self.bfs(gmap, items, pos, acc, maxdepth, currdepth + 1)
            val, _ = res
            if val > best:
                best = val
    ...
    def play_turn(
        ...
        if us.portal_gun:
            ...
            r, c = best_ind
            return f"{r},{c}"
        _, direction = self.bfs(gmap, items, us.location, 0, 5, 0)
        # print(best)
        goto = direction(us.location, gmap.size[0], gmap.size[1])
